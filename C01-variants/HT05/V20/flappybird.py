...
def main():
    ...
    """
    block_action_counter = pygame.time.get_ticks()
    pygame.init()
        ...
        for e in pygame.event.get():
            if e.type == QUIT or (e.type == KEYUP and e.key == K_ESCAPE):
                ...
                paused = not paused
                    ...
                    e.key in (K_UP, K_RETURN, K_SPACE)) and pygame.time.get_ticks() > block_action_counter:
                bird.msec_to_climb = Bird.CLIMB_DURATION
                block_action_counter = pygame.time.get_ticks() + (FPS * 30)
                #print("I blocked")
            # elif e.type == MOUSEBUTTONUP or (e.type == KEYUP and
